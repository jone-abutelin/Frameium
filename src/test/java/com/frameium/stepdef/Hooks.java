...
import com.frameium.AzureDevOps.AzureDevOpsFunctions;
...
import com.frameium.utilities.TestCaseInformation;
...
public class Hooks {
    ...
    public static String lighthousereport;
    ...
    //TestCaseInformation cucumberTestDetails = new TestCaseInformation();

    private TestNGCucumberRunner testNGCucumberRunner;
    String allureReportEnabled = getConfigProperty("allureReport");

    String azureDevOpsEnabled = getConfigProperty("azureDevOpsTestResultUpdate");





    public Hooks(TestSetUp setUp) throws IOException {
        System.out.println("<<<<<<<<<<  Hooks >>>>>>>>>>> ");
        this.setUp = setUp;
        setUp.baseTest.createExtendInstance();
        ObjectReader.reader = new PropertyReader();
        lighthousereport = ObjectReader.reader.getLightHouseReport().toLowerCase();
        accessibilityreport = ObjectReader.reader.getAccessibilityReport().toLowerCase();
        selfhealingoption = ObjectReader.reader.getSelfHealing().toLowerCase();
    }
    ...
    public JSONArray performAccessibility() throws Exception {
        ...
        JSONObject responseJson = new AXE.Builder(setUp.baseTest.driver, scriptUrl).analyze();
...
//		System.out.println(violation.getString("impact"));
    ...
    public void createAccessibilityReport(JSONArray violationsArray, int indexCount, int passedCount, int failedCount,
                                          int skippedCount, int sevCritical, int sevSerious, int sevModerate, int sevMinor, int totalViolations,String url)
            throws Exception {
        int totalVio = violationsArray.length();
        setUp.baseTest.driver.navigate().back();
...
//				bw.write("{  y: 2, legendText:\"Skipped 2\", color: \"Yellow\" }"+"\n");
            ...
            bw.write("<br> <br> <br>" + "\n");
            String pageUrl = setUp.baseTest.driver.getTitle();
...
//         indexCount++;
//
//        if (lighthousereport.equalsIgnoreCase("yes") && setUp.baseTest.driver != null) {
//
...
//        }
        ...
        if(setUp.baseTest.driver!=null)
        {
            setUp.baseTest.driver.quit();
        }

        if (allureReportEnabled != null && allureReportEnabled.equalsIgnoreCase("yes")) {
            GenericFunctions.generateAllureReport();
        }
    }
    /**
...
//	                }
    ...
    public void beforeScenario(Scenario scenario) {
        setUp.baseTest.initReports(scenario.getName());

        this.scenario = scenario;
        String ff = scenario.getName();
        TestCaseInformation.setScenarioName(scenario.getName());
        String SD = TestCaseInformation.getScenarioName();
    ...
    {
        TestCaseInformation.addTestEntry(method.getName());
        String SD = TestCaseInformation.getScenarioName();
        List<String> SdD = TestCaseInformation.getTestEntries();
    ...
    public void beforeEachStep(Scenario scenario) throws Exception {
        ...
        setUp.baseTest.infoLog(currentStepDescr);
    ...
    public void afterEveryStep(Scenario scenario) {
        ...
        if (stepStatus == "FAILED") {
            setUp.baseTest.stepFail("Failed");
            setUp.baseTest.getDriver().quit();
        } else if (stepStatus == "PASSED") {
            setUp.baseTest.stepPass("Passed");
        } else {
            setUp.baseTest.stepSkip("Step Skipped");
            setUp.baseTest.getDriver().quit();
        }
    }
    ...
    public String getError(Scenario scenario) {
        ...
        try {
            ...
            final Throwable error = (Throwable) getError.invoke(testCase);

            StackTraceElement[] errorArray = error.getStackTrace();
            for (StackTraceElement err : errorArray) {
                errorMsg = errorMsg + "\n" + err.toString();
            }

            System.out.println("finalErrorMsg ***** " + error.getMessage() + "\n" + errorMsg + "**** finalErrorMsg");
            finalErrorMsg = error.getMessage() + "\n" + errorMsg;
    ...
    public void afterScenario(Scenario scenario) {
        ...
        if (scenarioStatus == "FAILED") {
            setUp.baseTest.reportFailure(getError(scenario));
        } else if (scenarioStatus == "PASSED") {
            setUp.baseTest.reportPass("Scenario  :: " + scenario.getName() + " " + "::  Passed");
        } else {
            setUp.baseTest.reportSkip("Test Ignored");
        }
    }


    public void setLightHouseMetrix() throws Exception {

        if (lighthousereport.equalsIgnoreCase("yes") && setUp.baseTest.driver != null) {

            for (Map.Entry<String, String> entry : UrlTitle.entrySet()) {

                Allvalues.add(LighthouseMain(entry.getKey(), entry.getValue()));
            }
        }
    ...
    public String[] LighthouseMain(String currentUrl, String currentTitle) throws Exception {
        ...
        // clickElement(Page);
        WebDriverWait wait = new WebDriverWait(setUp.baseTest.driver, Duration.ofSeconds(200));
        ...
        setUp.baseTest.driver.get("https://googlechrome.github.io/lighthouse/viewer/?psiurl=" + currentUrl);

        @SuppressWarnings("deprecation")
        WebDriverWait wait1 = new WebDriverWait(setUp.baseTest.driver, Duration.ofSeconds(200));
...
//	    Thread.sleep(1000);
    ...
    public WebElement findElement(By byElement) {
        ...
        WebDriverWait wait = new WebDriverWait(setUp.baseTest.driver, Duration.ofSeconds(3000));
    ...
    public void takeScreenshot(Scenario scenario){
        final byte[] screenshot = ((TakesScreenshot) this.setUp.baseTest.driver).getScreenshotAs(OutputType.BYTES);
        scenario.attach(screenshot, "image/png", "image");
    }

    public void attachScreenshot(Scenario scenario, String filepath) throws IOException {
    ...
    public void addScreenshot(Scenario scenario){
...

        {
            if (selfhealingoption.equalsIgnoreCase("no")){
                final byte[] screenshot = ((TakesScreenshot) setUp.baseTest.driver).getScreenshotAs(OutputType.BYTES);
                scenario.attach(screenshot, "image/png", "image");

            }
            else {
                //driver2 = driver;
                //final byte[] screenshot = ((TakesScreenshot) driver2.getDelegate()).getScreenshotAs(OutputType.BYTES);
                //scenario.attach(screenshot, "image/png", "image");
            }
        ...
        else if(setUp.baseTest.driver!=null)
        {
            final byte[] screenshot = ((TakesScreenshot) setUp.baseTest.driver).getScreenshotAs(OutputType.BYTES);
            scenario.attach(screenshot, "image/png", "image");
    ...
    /**
     * This method used for capture screen in android driver
     */
    public String captureScreen(String fileName, WebDriver androiddriver) {
        ...
        File screFile = ((TakesScreenshot) androiddriver).getScreenshotAs(OutputType.FILE);
        try {
    ...
    //---------------------------- Azure DevOps Functions to Update test Case outcome ------------------------------------

    @After
    public void updateTestOutcome(Scenario scenario) {
        if (azureDevOpsEnabled != null && azureDevOpsEnabled.equalsIgnoreCase("yes")) {
            switch (scenario.getStatus()) {
                case PASSED:
                    log.info("\u2705 Passed scenario : " + scenario.getName());
                    AzureDevOpsFunctions.updateTestResultInAzureDevOps(scenario, "passed");
                    break;

                case FAILED:
                    log.info("\u274C Failed scenario : " + scenario.getName());
                    AzureDevOpsFunctions.updateTestResultInAzureDevOps(scenario, "failed");
                    break;

                case UNDEFINED:
                    log.info("\u2753 Undefined scenario : " + scenario.getName());
                    AzureDevOpsFunctions.updateTestResultInAzureDevOps(scenario, "unspecified");
                    break;

                case SKIPPED:
                    if (scenario.isFailed()) {
                        log.info("\u274C Skipped scenario : " + scenario.getName());
                        AzureDevOpsFunctions.updateTestResultInAzureDevOps(scenario, "failed");
                    } else {
                        log.info("\u23F8 Skipped scenario : " + scenario.getName());
                        AzureDevOpsFunctions.updateTestResultInAzureDevOps(scenario, "paused");
                    }
                    break;

                case PENDING:
                    log.info("\u23F3 Pending scenario : " + scenario.getName());
                    AzureDevOpsFunctions.updateTestResultInAzureDevOps(scenario, "notExecuted");
                    break;

                default:
                    log.error("\u26A0 Unexpected scenario status: " + scenario.getStatus());
                    AzureDevOpsFunctions.updateTestResultInAzureDevOps(scenario, "error");
                    break;
            }
        }
    }
